...
public sealed partial class WeatherCelestialPreviewSettingViewModel(IServiceProvider provider, IServiceFactory factory, IMessenger messenger, IDisposer disposer, IDispatcher dispatcher, WeatherSettings settings, IWritableOptions<WeatherSettings> writer) :
    ModuleSettingViewModel<WeatherSettings, int>(provider, factory, messenger, disposer, dispatcher, settings, writer, "Weather", 60, config => (int)config.PreviewCelestial, (config, value) => config.PreviewCelestial = Enum.IsDefined(typeof(WeatherCelestial), value) ? (WeatherCelestial)value : WeatherCelestial.Live);
...
public sealed partial class WeatherEffectPreviewSettingViewModel(IServiceProvider provider, IServiceFactory factory, IMessenger messenger, IDisposer disposer, IDispatcher dispatcher, WeatherSettings settings, IWritableOptions<WeatherSettings> writer) :
    ModuleSettingViewModel<WeatherSettings, int>(provider, factory, messenger, disposer, dispatcher, settings, writer, "Weather", 70, config => (int)config.PreviewEffect, (config, value) => config.PreviewEffect = Enum.IsDefined(typeof(WeatherEffect), value) ? (WeatherEffect)value : WeatherEffect.Live);
...
public sealed partial class WeatherTemperaturePreviewSettingViewModel(IServiceProvider provider, IServiceFactory factory, IMessenger messenger, IDisposer disposer, IDispatcher dispatcher, WeatherSettings settings, IWritableOptions<WeatherSettings> writer) :
    ModuleSettingViewModel<WeatherSettings, int>(provider, factory, messenger, disposer, dispatcher, settings, writer, "Weather", 80, config => (int)config.PreviewTemperature, (config, value) => config.PreviewTemperature = Enum.IsDefined(typeof(WeatherTemperature), value) ? (WeatherTemperature)value : WeatherTemperature.Live);
