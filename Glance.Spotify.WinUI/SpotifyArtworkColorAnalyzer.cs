...
internal static class SpotifyArtworkColorAnalyzer
{
    private const uint SampleSize = 32;

    public static async Task<SpotifyArtworkColors> AnalyzeAsync(IRandomAccessStream stream)
    {
        stream.Seek(0);
        ...
        if (dominant.Count == 0)
        {
            return new SpotifyArtworkColors(SpotifyViewModel.DefaultAccentColor, averageColor);
        }

        uint accentColor = Normalize(dominant.Red / dominant.Count,
            dominant.Green / dominant.Count,
            dominant.Blue / dominant.Count);
        return new SpotifyArtworkColors(accentColor, averageColor);
    }
...
internal readonly record struct SpotifyArtworkColors(uint AccentColor,
    uint AverageColor);
