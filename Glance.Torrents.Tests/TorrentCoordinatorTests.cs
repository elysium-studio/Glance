...
public sealed class TorrentCoordinatorTests
{
    ...
    public async Task ConfirmationStartsSelectedFiles()
    {
        ...
        await coordinator.ConfirmAsync(session, ["one.txt"], @"C:\Downloads\Other");
        Assert.Equal(session.SessionId, engine.ConfirmedSession);
        Assert.Equal(["one.txt"], engine.SelectedFiles);
        Assert.Equal(@"C:\Downloads\Other", engine.DownloadPath);
    }
    ...
    private sealed class FakeEngine : ITorrentEngineService
    {
        ...
        public Task ConfirmAsync(string sessionId, IReadOnlyCollection<string> selectedFiles, string downloadPath, CancellationToken cancellationToken = default) { ConfirmedSession = sessionId; SelectedFiles = selectedFiles; DownloadPath = downloadPath; return Task.CompletedTask; }
